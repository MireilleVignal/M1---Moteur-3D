...
void inverseMatrice(Vect3 mat[3], Vect3 result[3]) {
	...
	det = mat[0].x * mat[1].y * mat[2].z + mat[0].y * mat[1].z * mat[2].x + mat[1].x * mat[2].y * mat[0].z 
			- (mat[0].z * mat[1].y * mat[2].x + mat[0].y * mat[1].x * mat[2].z + mat[0].x * mat[1].z * mat[2].y);
...
/*	for (int i : {0,1,2}) {
			std::cerr << tri[i].x << " " << tri[i].y << " " << tri[i].z << std::endl;
	}
...
//	Vect3 mat[3] = {Vect3(0, 0, 0), Vect3(0, 0, 0), Vect3(0, 0, 0)};
//	inverseMatrice(tri, mat);
//	std::cerr << mat[0] << std::endl;
	//Vect3 mat[3] = {Vect3(a.x, a.y, 1), Vect3(b.x, b.y, 1), Vect3(c.x, c.y, 1)};
	...
	//		std::cerr << baricentre.x << " " << baricentre.y << " " << baricentre.z << std::endl;
	...
	//		else {
				...
				//P.z = std::max(a.z, std::max(b.z, c.z));
